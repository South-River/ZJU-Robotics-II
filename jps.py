...
class GridMap(object):
    ...
    def setOccupiedGrid(self):
        ...
        for obstacle in self.obstacle_list:
            obstacle_buttom_left_map_coor = self.transfer2MapCoor(obstacle.x - 4 * self.robot_size,
                                                                  obstacle.y - 4 * self.robot_size)
            obstacle_top_right_map_coor = self.transfer2MapCoor(obstacle.x + 4 * self.robot_size,
                                                                obstacle.y + 4 * self.robot_size)
