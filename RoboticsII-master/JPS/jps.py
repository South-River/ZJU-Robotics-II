...
class GridMap(object):
    ...
    def setOccupiedGrid(self):
        ...
        for obstacle in self.obstacle_list:
            obstacle_buttom_left_map_coor = self.transfer2MapCoor(obstacle.x - 2 * self.robot_size,
                                                                  obstacle.y - 2 * self.robot_size)
            obstacle_top_right_map_coor = self.transfer2MapCoor(obstacle.x + 2 * self.robot_size,
                                                                obstacle.y + 2 * self.robot_size)
            for i in range((int)(obstacle_buttom_left_map_coor.i), (int)(obstacle_top_right_map_coor.i + 1)):
                for j in range((int)(obstacle_buttom_left_map_coor.j), (int)(obstacle_top_right_map_coor.j + 1)):
                    if i >= 0 and i < self.col and j >= 0 and j < self.row:
                        self.map[j][i] = 1
                        self.obstacle_x.append(int(i * self.grid_size + self.min_x + self.grid_size // 2))
                        self.obstacle_y.append(int(j * self.grid_size + self.min_y + self.grid_size // 2))
                        #debugger.draw_point(int(i * self.grid_size + self.min_x + self.grid_size // 2), int(j * self.grid_size + self.min_x + self.grid_size // 2))
    ...
    def isForcedNeighbor(self, neighbor, current_node):
        ...
        if direction.i != 0 and direction.j != 0:
            if self.grid_map.map[(neighbor.j + current_node.j - direction.j) // 2][
                (neighbor.i + current_node.i - direction.i) // 2] == 1 and self.grid_map.map[neighbor.j][neighbor.i] == 0\
                and self.grid_map.map[neighbor.j][neighbor.i] != 9:
        ...
        if direction.i == 0:
            if neighbor.i >= 0 and neighbor.i < self.grid_map.row:
                if self.grid_map.map[current_node.j][neighbor.i] == 1 and self.grid_map.map[neighbor.j][neighbor.i] == 0\
                    and self.grid_map.map[neighbor.j][neighbor.i] != 9:
                    self.grid_map.map[current_node.j][current_node.i] = 9
                    return True
            else:
        ...
        if direction.j == 0:
            if neighbor.j >= 0 and neighbor.j < self.grid_map.row:
                if self.grid_map.map[neighbor.j][current_node.i] == 1 and self.grid_map.map[neighbor.j][neighbor.i] == 0\
                        and self.grid_map.map[neighbor.j][neighbor.i] != 9:
                    self.grid_map.map[current_node.j][current_node.i] = 9
                    return True
            else:
